        ...
        {tab === 'builder' && bp && (
          <div style={{ padding: '32px 36px' }}>
            ...
            <button className="btn-ghost" style={{ width: '100%', justifyContent: 'center', marginBottom: '28px' }}
              onClick={async () => {
                const numero = bp.semanas.length + 1
                if (bp.id) {
                  // Plan ya existe — crear semana en DB inmediatamente
                  const { data: newSem } = await supabase.from('semanas').insert({ plan_id: bp.id, numero }).select().single()
                  if (newSem) {
                    setBp((p: any) => ({ ...p, semanas: [...p.semanas, { id: newSem.id, numero, dias: [] }] }))
                    return
                  }
                }
                // Plan nuevo — id temporal
                setBp((p: any) => ({ ...p, semanas: [...p.semanas, { id: uid(), numero, dias: [] }] }))
              }}>
