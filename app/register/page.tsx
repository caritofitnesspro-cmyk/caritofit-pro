...
export default function RegisterPage() {
  ...
  function addSugerencia(s: string) {
  ...
  function validateStep2() {
    ...
    if (!form.objetivo.trim() || form.objetivo.length < 10) {
      setError('Describí tu objetivo con un poco más de detalle — es la base para armar tu plan')
      return
    }
  ...
  async function handleRegister() {
    ...
    try {
      ...
      // Completar el perfil con los datos adicionales
      if (authData.user) {
      ...
      <div style={{ background: '#faf8f7', borderRadius: '28px', padding: '40px', width: '100%', maxWidth: '560px', boxShadow: '0 24px 80px rgba(0,0,0,.3)', margin: 'auto' }}>
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                  <div style={{
                    ...
                    fontSize: '12px', fontWeight: '700', transition: '.3s',
                ...
                {i < 2 && <div style={{ width: '48px', height: '2px', background: step > i + 1 ? '#B05276' : '#d5c4c8', marginTop: '15px', flexShrink: 0, transition: '.3s' }} />}
              </div>
            ))}
          </div>
        )}
        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
              {[['nombre', 'Nombre *', 'Ej: María'], ['apellido', 'Apellido *', 'Ej: García']].map(([k, l, ph]) => (
                <div key={k}>
                  ...
                  <input className="input-field" type="text" placeholder={ph} value={(form as any)[k]} onChange={e => set(k, e.target.value)} />
                </div>
              ))}
            </div>

            <div style={{ marginBottom: '14px' }}>
              <label className="field-label">DNI * <span style={{ textTransform: 'none', fontSize: '11px', fontWeight: '400', letterSpacing: 0, color: '#8a7070' }}>(sin puntos ni guiones)</span></label>
              <input className="input-field" type="text" placeholder="Ej: 38500001" maxLength={8}
                value={form.dni} onChange={e => set('dni', e.target.value.replace(/\D/g, ''))} />
            </div>

            <div style={{ marginBottom: '14px' }}>
              <label className="field-label">Email *</label>
              <input className="input-field" type="email" placeholder="tu@mail.com" value={form.email} onChange={e => set('email', e.target.value)} />
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
              <div>
                <label className="field-label">Teléfono</label>
                <input className="input-field" type="text" placeholder="11-0000-0000" value={form.telefono} onChange={e => set('telefono', e.target.value)} />
              </div>
              <div>
                <label className="field-label">Edad</label>
                <input className="input-field" type="number" placeholder="30" value={form.edad} onChange={e => set('edad', e.target.value)} />
              </div>
            </div>
            ...
            <div style={{ marginBottom: '14px' }}>
              <label className="field-label">Restricciones / Lesiones <span style={{ textTransform: 'none', fontSize: '11px', fontWeight: '400', letterSpacing: 0, color: '#8a7070' }}>(opcional)</span></label>
        ...
        {/* PASO 2: OBJETIVO LIBRE */}
        {step === 2 && (
          <div>
            <div style={{ fontFamily: 'Georgia, serif', fontSize: '22px', fontWeight: '900', color: '#7D0531', marginBottom: '4px' }}>¿Cuál es tu objetivo?</div>
            <div style={{ fontSize: '14px', color: '#8a7070', marginBottom: '20px', lineHeight: '1.5' }}>
              Contanos con tus palabras qué querés lograr. <strong style={{ color: '#7D0531' }}>Tu profesora va a armar tu plan personalizado en base a esto.</strong>
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '7px', marginBottom: '10px' }}>
              {OBJ_SUGERENCIAS.map(s => (
                <span key={s} onClick={() => addSugerencia(s)}
                  style={{ padding: '6px 13px', borderRadius: '20px', background: '#ede0e2', border: '1.5px solid #d5c4c8', fontSize: '12px', fontWeight: '600', color: '#5a2a3a', cursor: 'pointer', transition: '.15s' }}
                  onMouseEnter={e => { (e.target as HTMLElement).style.background = '#DBBABF'; (e.target as HTMLElement).style.borderColor = '#B05276' }}
                  onMouseLeave={e => { (e.target as HTMLElement).style.background = '#ede0e2'; (e.target as HTMLElement).style.borderColor = '#d5c4c8' }}
                >{s}</span>
              ))}
            </div>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <textarea
                ...
                placeholder="Escribí libremente tu objetivo. Por ejemplo: quiero bajar 5 kilos antes del verano, mejorar mi resistencia para correr una carrera de 5km, recuperarme de una lesión de rodilla..."
                style={{ background: '#ede0e2', border: '2px solid #d5c4c8', borderRadius: '14px', padding: '16px', fontSize: '15px', color: '#2a1520', outline: 'none', width: '100%', fontFamily: 'inherit', resize: 'vertical', minHeight: '110px', lineHeight: '1.6', transition: '.2s' }}
                onFocus={e => { e.target.style.borderColor = '#B05276'; e.target.style.background = '#faf8f7' }}
                onBlur={e => { e.target.style.borderColor = '#d5c4c8'; e.target.style.background = '#ede0e2' }}
              />
              <div style={{ fontSize: '11px', color: '#8a7070', marginTop: '5px', fontStyle: 'italic' }}>✏️ Podés tocar las etiquetas de arriba para agregar ideas, o escribir directamente.</div>
            </div>

            <div style={{ marginBottom: '20px' }}>
              <label className="field-label">Nivel de actividad física</label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                {[
                  { n: 'Principiante', icon: '🌱', desc: 'Empiezo ahora o hace mucho que no entreno' },
                  { n: 'Intermedio', icon: '🔥', desc: 'Entreno regularmente hace más de 6 meses' },
                  { n: 'Avanzado', icon: '⚡', desc: 'Entreno intensamente hace años' },
                ].map(({ n, icon, desc }) => (
                  <div key={n} onClick={() => set('nivel', n)}
                    style={{ border: `2px solid ${form.nivel === n ? '#7D0531' : '#d5c4c8'}`, borderRadius: '14px', padding: '14px 10px', textAlign: 'center', cursor: 'pointer', transition: '.2s', background: form.nivel === n ? '#f5eaed' : '#faf8f7' }}>
                    <div style={{ fontSize: '28px', marginBottom: '6px' }}>{icon}</div>
                    <div style={{ fontWeight: '700', fontSize: '13px' }}>{n}</div>
                    <div style={{ fontSize: '11px', color: '#8a7070', marginTop: '3px', lineHeight: '1.3' }}>{desc}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {step === 3 && (
          <div>
            ...
            <div style={{ marginBottom: '14px' }}>
              <label className="field-label">Contraseña * <span style={{ textTransform: 'none', fontWeight: '400', letterSpacing: 0, fontSize: '11px', color: '#8a7070' }}>(mínimo 6 caracteres)</span></label>
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <input className="input-field" type="password" placeholder="Igual que la anterior" value={form.password2} onChange={e => set('password2', e.target.value)} onKeyDown={e => e.key === 'Enter' && handleRegister()} />
              {form.password2 && (
                <div style={{ fontSize: '12px', marginTop: '4px', color: form.password === form.password2 ? '#16a34a' : '#c0392b' }}>
                  {form.password === form.password2 ? '✓ Coinciden' : '✗ No coinciden'}
                </div>
              )}
            </div>
        ...
        {step === 4 && (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <p style={{ fontSize: '15px', color: '#5a2a3a', marginBottom: '24px', lineHeight: '1.6' }}>
              Tu cuenta fue creada. La profesora va a revisar tu objetivo y armar tu plan personalizado. ¡Te avisará cuando esté listo!
            </p>
